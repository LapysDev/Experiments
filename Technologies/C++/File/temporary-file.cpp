...
int main(void) {
    // Constant > File ...
    char const* const fileName = ::tmpnam(NULL); // NOTE (Lapys) -> Also acts as the file`s pathname.
    ...
    if (-1 == fileDescriptor) {
        ...
        switch (errno) {
            case EACCES: error_code_syntax = (char*) "EACCES"; error_description = (char*) "The requested access to the file is not allowed;\n\r\tor search permission is denied for one of the directories in the path prefix of pathname;\n\r\tor the file did not exist yet and write access to the parent directory is not allowed"; break;
            case EAGAIN: error_code_syntax = (char*) "EAGAIN"; error_description = (char*) "File pathname names the slave side of a pseudo-terminal device that is locked"; break;
            case EBADF: error_code_syntax = (char*) "EBADF"; error_description = (char*) "File specified with an invalid file descriptor"; break;
            case EEXIST: error_code_syntax = (char*) "EEXIST"; error_description = (char*) "File pathname already exists and `O_CREAT` and `O_EXCL` file descriptors were used"; break;
            case EFAULT: error_code_syntax = (char*) "EFAULT"; error_description = (char*) "File pathname points outside your accessible address space"; break;
            case EFBIG /* -> `EOVERFLOW` */: error_code_syntax = (char*) "EOVERFLOW"; error_description = (char*) "File pathname refers to a regular file that is too large to be opened"; break;
            ...
            case EINVAL: error_code_syntax = (char*) "EINVAL"; error_description = (char*) "The filesystem does not support the specified file descriptor (e.g.: `O_DIRECT`);\n\r\tor `O_TMPFILE` was specified in flags, but neither `O_RDWR` nor `O_WRONLY` was specified;\n\r\tor the file descriptor specified `O_CREAT` and the final component (\"basename\") of the new file's pathname is invalid"; break;
            case EIO: error_code_syntax = (char*) "EIO"; error_description = (char*) "File pathname names a `STREAMS` file and a hangup or error occurred during the function invocation"; break;
            case EISDIR: error_code_syntax = (char*) "EISDIR"; error_description = (char*) "File pathname refers to a directory and the access requested involved writing;\n\r\tor `O_TMPFILE` and one of `O_RDWR` or `O_WRONLY` were specified file descriptors, but this kernel version does not provide the `O_TMPFILE` functionality"; break;
            ...
            case ENAMETOOLONG: error_code_syntax = (char*) "ENAMETOOLONG"; error_description = (char*) "File pathname was too long"; break;
            ...
            case ENODEV: error_code_syntax = (char*) "ENODEV"; error_description = (char*) "File pathname refers to a device special file and no corresponding device exists"; break;
            case ENOENT: error_code_syntax = (char*) "ENOENT"; error_description = (char*) "`O_CREAT` file descriptor is not set and the named file does not exist;\n\r\tor a directory component in pathname does not exist or is a dangling symbolic link;\n\r\tor file pathname refers to a nonexistent directory;\n\r\tor `O_TMPFILE` and one of `O_RDWR` or `O_WRONLY` were specified file descriptors, but this kernel version does not provide the `O_TMPFILE` functionality"; break;
            ...
            case ENOSPC: error_code_syntax = (char*) "ENOSPC"; error_description = (char*) "File pathname was to be created but the device containing pathname has no room for the new file"; break;
            case ENOTDIR: error_code_syntax = (char*) "ENOTDIR"; error_description = (char*) "A component used as a directory in pathname is not a valid directory;\n\r\tor the `O_DIRECTORY` file descriptor was specified and pathname was not a directory"; break;
            ...
            case EROFS: error_code_syntax = (char*) "EROFS"; error_description = (char*) "File pathname refers to a file on a read-only filesystem and write access was requested"; break;
        }
