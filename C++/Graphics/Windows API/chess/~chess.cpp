...
namespace Game {
    ...
    static unsigned char MEMORY[/* 275 ÷ CHAR_BIT */ 35] = {0};
...
void Game::clearData(void) {
    for (
        unsigned char *iterator = Game::MEMORY + (sizeof(Game::MEMORY) / sizeof(unsigned char));
        iterator-- != Game::MEMORY;
    ) *iterator = 0x000000u;
}
...
unsigned char Piece::getColumn(void) const {
    if (Piece::BISHOP == this -> getType()) return ((this -> getPosition() * 2u) % Game::Tiles::COLUMN_COUNT) + (this -> getRow() % 2u
        ? (Piece::WHITE == this -> getColor() ? 0 : 1) == this -> getIndex()
        : (Piece::WHITE == this -> getColor() ? 1 : 0) == this -> getIndex()
    );
...
unsigned char Piece::getPosition(void) const {
    ...
    switch (type) {
        case Piece::BISHOP: return *(this -> getData()) & 0x1Fu;
        case Piece::KNIGHT: case Piece::PAWN: case Piece::ROOK: return *(this -> getData()) & 0x3Fu;
        case Piece::KING: case Piece::QUEEN: {
            Color const color = this -> getColor();
            return (
                ((*Game::getPiece(color, Piece::BISHOP, Piece::QUEEN == type ? 1u : 0u).getData() >> 6u) << 4u) |
                ((*Game::getPiece(color, Piece::KNIGHT, Piece::QUEEN == type ? 1u : 0u).getData() >> 6u) << 2u) |
                ((*Game::getPiece(color, Piece::ROOK  , Piece::QUEEN == type ? 1u : 0u).getData() >> 6u) << 0u)
            );
        } break;
    }
...
unsigned char Piece::getRow(void) const {
    if (Piece::BISHOP == this -> getType()) return this -> getPosition() / (Game::Tiles::COLUMN_COUNT / 2u);
...
void Piece::setPosition(unsigned char const column, unsigned char const row) {
    ...
    switch (type) {
        case Piece::BISHOP: {
            ...
            while (position--) {
                ...
                unsigned char const positionColumn = ((position * 2u) % Game::Tiles::COLUMN_COUNT) + (positionRow % 2u
                    ? (Piece::WHITE == color ? 0 : 1) == index
                    : (Piece::WHITE == color ? 1 : 0) == index
                );
        ...
        case Piece::KING: case Piece::QUEEN: {
            ...
            *position[1] |= (((column & 1u) << 1u) | ((row >> 2u) << 0u)) << 6u;
...
void UPDATE(void) {
    ...
    static unsigned short const tileHeight    = Game::Board::HEIGHT / Game::Tiles::ROW_COUNT   ;
    static unsigned short const tileWidth     = Game::Board::WIDTH  / Game::Tiles::COLUMN_COUNT;
    ...
    switch (Program::THREAD_MESSAGE.message) {
        ...
        case WM_PAINT: {
            // ... ->> Tiles (and Cursor)
            static unsigned short const boardLeft    = (Window::WIDTH  - Game::Board::WIDTH ) / 2u;
            static unsigned short const boardTop     = (Window::HEIGHT - Game::Board::HEIGHT) / 2u;
            ...
            // ... ->> Pieces
            union PiecesEnumerationCallback { static Game::PiecesEnumerationControl value(Piece const piece) {
                unsigned char const tileColumn = piece.getColumn();
                unsigned char const tileRow = piece.getRow();
                ...
                for (unsigned short tileY = tileHeight; tileY--; ) {
                    ...
                    // Game::Pieces::BITMAP_MEMORY[x + (y * Game::Pieces::BITMAP.bmWidth)]
                    putPixel(x, y, 0x0000FFu);
                }
            ...
            Game::enumeratePieces(&PiecesEnumerationCallback::value);
