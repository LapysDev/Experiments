...
/* Function */
    // Draw Ellipse
    void drawEllipse(const HWND windowHandle, const HDC graphicsDeviceContextHandle, const unsigned short horizontalRadius, const unsigned short verticalRadius, const COLORREF color) noexcept {
        // Initialization > (X, Y)
        short x;
        short y;
        ...
        for (y = -verticalRadius; y <= verticalRadius; ++y) {
            // Initialization > (Coordinate, Distance)
            int coordinate = ((x * x) * (verticalRadius * verticalRadius)) + ((y * y) * (horizontalRadius * horizontalRadius));
            unsigned int distance = (horizontalRadius * horizontalRadius) * (verticalRadius * verticalRadius);

            // Logic > ...
            if (((unsigned long) coordinate) <= distance)
            ::putPixel(windowHandle, graphicsDeviceContextHandle, x, y, color);
        }
    }
    inline void drawEllipse(const HWND windowHandle, const HDC graphicsDeviceContextHandle, const COLORREF color) noexcept { ::drawEllipse(windowHandle, graphicsDeviceContextHandle, LENGTH, LENGTH, color); }
    ...
    LRESULT CALLBACK windowProcedure(HWND windowHandle, UINT message, WPARAM messageParameter, LPARAM additionalMessageParameter) noexcept {
        ...
        switch (message) {
            ...
            case WM_PAINT: {
                ...
                ::drawEllipse(windowHandle, windowGraphicsDeviceContextHandle, LENGTH, LENGTH / 2u, RGB(240, 240, 240));
...
int WinMain(HINSTANCE instanceHandle, HINSTANCE /*previousInstanceHandle*/, LPSTR /*programInitiationCommandLine*/, int windowAppearance) {
    ...
    /* Initialization */
        ...
        WNDCLASS windowClass {CS_HREDRAW | CS_VREDRAW, &windowProcedure, 0, 0, instanceHandle, 0x0, (HCURSOR) ::LoadCursor(0x0, IDC_ARROW), (HBRUSH) COLOR_WINDOW + 1, 0x0, "Ellipse"};
    ...
    if (::RegisterClass(&windowClass)) {
        ...
        windowHandle = ::CreateWindowEx(0, windowClass.lpszClassName, "Ellipse", WS_OVERLAPPEDWINDOW | WS_VISIBLE, (::GetSystemMetrics(SM_CXSCREEN) - (ORIGIN * 2u)) / 2, (::GetSystemMetrics(SM_CYSCREEN) - (ORIGIN * 2u)) / 2, ORIGIN * 2u, ORIGIN * 2u, 0x0, 0x0, instanceHandle, 0x0);
