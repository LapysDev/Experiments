/* Logic */
    ...
    /* Directives */
    #pragma comment (lib, "d3d9.lib") // NOTE (Lapys) -> Include the Direct 3D library file.
    ...
    namespace Lapys {
        ...
        /* Namespace */
            ...
            namespace Action {
                namespace Direct3D {
                    const D3DSWAPEFFECT CopyFrames = D3DSWAPEFFECT_COPY;
                    const D3DSWAPEFFECT DiscardOldFrames = D3DSWAPEFFECT_DISCARD;
                    const D3DSWAPEFFECT DiscardAndProtectOldFrames = D3DSWAPEFFECT_FLIP;
                }

                namespace PeekMessage {
                    const unsigned int Discard = PM_REMOVE;
                    const unsigned int MouseAndKeyboardOnly = PM_QS_INPUT/* || (QS_INPUT << 16) */;
                    const unsigned int PaintOnly = PM_QS_PAINT/* || (QS_PAINT << 16) */;
                    const unsigned int PostedMessagesOnly = PM_QS_POSTMESSAGE/* || ((QS_POSTMESSAGE | QS_HOTKEY | QS_TIMER) << 16) */;
                    const unsigned int SentMessagesOnly = PM_QS_SENDMESSAGE/* || (QS_SENDMESSAGE << 16) */;
                    const unsigned int Persist = PM_NOREMOVE;
                }
            }
            ...
            namespace Event {
                ...
                namespace Window {
                    ...
                    const unsigned int Paint = WM_PAINT;
                    ...
                    const unsigned int Timer = WM_TIMER;
            ...
            namespace Settings {
                /* Global > ... */
                bool flushOutputBufferOnNewline = true;
            }
        ...
        /* Namespace */
            ...
            namespace Console {
                /* Functions */
                    ...
                    void Write(const int Message) { ::printf("%i", Message); }

                    // Write Line
                    void WriteLine(const char* Message) { ::puts(Message); Lapys::Settings::flushOutputBufferOnNewline && Lapys::Console::FlushOutputBuffer(); }
                    void WriteLine(const int Message) { ::printf("%i\n", Message); Lapys::Settings::flushOutputBufferOnNewline && Lapys::Console::FlushOutputBuffer(); }
            }
            ...
            namespace Data {
                /* Functions, Global */
                    ...
                    // Create Direct 3D Present Parameters
                    const D3DPRESENT_PARAMETERS& CreateDirect3DPresentParameters(HWND HandlerWindow) {
                        // Constant > Direct 3D Present Parameters --- NOTE (Lapys) -> Data on (Graphics) Device Information.
                        static D3DPRESENT_PARAMETERS DIRECT3D_PRESENT_PARAMETERS;

                        // Deletion
                        ::ZeroMemory(&DIRECT3D_PRESENT_PARAMETERS, sizeof(DIRECT3D_PRESENT_PARAMETERS));

                        // Modification > Direct 3D Present Parameters > ... --- REDACT (Lapys)
                        DIRECT3D_PRESENT_PARAMETERS.hDeviceWindow = HandlerWindow;
                        DIRECT3D_PRESENT_PARAMETERS.SwapEffect = Lapys::Action::Direct3D::DiscardOldFrames;
                        DIRECT3D_PRESENT_PARAMETERS.Windowed = true;

                        // Return
                        return DIRECT3D_PRESENT_PARAMETERS;
                    }
                    ...
                    // Initiate Direct 3D
                    const LPDIRECT3D9& InitiateDirect3D(LPDIRECT3D9& Direct3DPointer, LPDIRECT3DDEVICE9& Direct3DDevicePointer, HWND HandlerWindow) {
                        // // Constant > Direct 3D Present Parameters
                        // D3DPRESENT_PARAMETERS DIRECT3D_PRESENT_PARAMETERS = Lapys::Data::CreateDirect3DPresentParameters(HandlerWindow);

                        // // Update > Direct 3D Pointer; ...
                        // Direct3DPointer = ::Direct3DCreate9(D3D_SDK_VERSION);
                        // Direct3DPointer -> CreateDevice(
                        //     D3DADAPTER_DEFAULT /* NOTE (Lapys) -> Direct 3D Adapter -> Default */,
                        //     D3DDEVTYPE_HAL /* NOTE (Lapys) -> Direct 3D Device Type -> Hardware Abstraction Layer */,
                        //     HandlerWindow,
                        //     _STRICT = D3DCREATE_SOFTWARE_VERTEXPROCESSING /* NOTE (Lapys) -> Use Software to Process Vertexes. */,
                        //     &DIRECT3D_PRESENT_PARAMETERS, &Direct3DDevicePointer
                        // );

                        // // Return
                        // return Direct3DPointer;
                    }
                    ...
                    MSG& RunMessageQueue(MSG& Message, const bool REMOVE_QUEUED_MESSAGES = true) {
                        // Initialization > Anticipate Next Message
                        bool anticipateNextMessage = true;

                        // Loop
                        while (anticipateNextMessage) {
                            /* Loop
                                --- NOTE ---
                                    #Lapys: The difference between `PeekMessage` and `GetMessage` is that to enable realtime processing (not strictly event-driven)
                                        we peek at the next message in the event queue rather than wait for one.
                            */
                            while (anticipateNextMessage && ::PeekMessage(&Message, NULL, 0, 0, REMOVE_QUEUED_MESSAGES ? Lapys::Action::PeekMessage::Discard : Lapys::Action::PeekMessage::Persist)) {
                                // (Translate, Dispatch) Message > Message
                                ::TranslateMessage(&Message);
                                ::DispatchMessage(&Message);

                                // Update > Anticipate Next Message
                                anticipateNextMessage = Message.message != Lapys::Event::Window::Quit;
                            }
                        }
                    ...
                    // Terminate Direct 3D
                    void TerminateDirect3D(LPDIRECT3D9& Direct3DPointer, LPDIRECT3DDEVICE9& Direct3DDevicePointer) {
                        // Deletion
                        Direct3DDevicePointer -> Release();
                        Direct3DPointer -> Release();
                    }
            }
