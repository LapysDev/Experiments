...
template <std::size_t length, unsigned long long radix = 10uLL>
union factors_t {
  long double const value[length];

  template <typename... types>
  constexpr factors_t(typename conditional_t<length == 1u + sizeof...(types), long double>::type const value, types const... values) noexcept :
    value{value, values...}
  {}

  template <typename... types>
  constexpr factors_t(typename conditional_t<length != 1u + sizeof...(types), long double>::type const value, types const... values) noexcept :
    factors_t<length, radix>::factors_t(value * static_cast<long double>(radix), value, values...)
  {}
};

template <unsigned long long radix>
union factors_t<0u, radix> {
  long double const value[1];
  constexpr factors_t(...) noexcept : value{1.0L} {}
};

void print() {}
template <std::size_t capacity, unsigned long long radix, typename... types>
void print(factors_t<capacity, radix> const& factors, types const&... more) {
  std::cout << "[]: {" << std::scientific;
    for (std::size_t iterator = 0u; capacity != iterator; ++iterator)
    std::cout << factors.value[iterator] << (1u == capacity - iterator ? "" : ", ");
  std::cout << "}" "\r\n";
  print(more...);
}

template <unsigned long long>
constexpr long double lastof() {
  return 1.0L;
}

template <unsigned long long radix, std::size_t segment, std::size_t... segments>
constexpr typename conditional_t<0u == sizeof...(segments), long double>::type lastof(factors_t<segment, radix> const& factors, factors_t<segments, radix> const&...) {
  return *factors.value * static_cast<long double>(radix);
}

template <unsigned long long radix, std::size_t segment, std::size_t... segments>
constexpr typename conditional_t<0u != sizeof...(segments), long double>::type lastof(factors_t<segment, radix> const&, factors_t<segments, radix> const&... more) {
  return lastof(more...);
}

template <unsigned long long radix, std::size_t... integers, std::size_t... indexes, std::size_t... segments>
typename conditional_t<(LDBL_MAX_10_EXP <= sizeof...(integers)), unsigned short>::type add(
  collection<std::size_t, integers...> const = {},
  index_sequence<0u, indexes...> const = {},
  factors_t<segments, radix> const&... factors
) {
  print(factors...);
...
template <unsigned long long radix, std::size_t... integers, std::size_t... indexes, std::size_t... segments>
typename conditional_t<(LDBL_MAX_10_EXP > sizeof...(integers)), unsigned short>::type add(
  collection<std::size_t, integers...> const = {}, index_sequence<0u, indexes...> const = {},
  factors_t<segments, radix> const&... factors
) {
  std::cout << "[...]: " << sizeof...(integers) << ", " << sizeof...(indexes) << std::endl;
  return (LDBL_MAX_10_EXP < DBL_MAX_10_EXP + sizeof...(integers) && DBL_MAX_10_EXP == sizeof...(indexes)
    ? add<radix>(collection<std::size_t, integers...>(), index_sequence<(DBL_MAX_10_EXP >= LDBL_MAX_10_EXP - sizeof...(integers) ? LDBL_MAX_10_EXP - sizeof...(integers) : DBL_MAX_10_EXP)>(), factors...)
    : add<radix>(collection<std::size_t, integers..., (indexes + sizeof...(integers))...>(), index_sequence<DBL_MAX_10_EXP>(), factors..., factors_t<sizeof...(indexes), radix>(
      lastof<radix>(factors...)
    ))
  );
}
