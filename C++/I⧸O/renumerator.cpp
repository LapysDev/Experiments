...
# include <stdint.h>    // --> uint32_t ->> will be defined #include <stdlib.h>    // --> ::realpath(…)
...
struct renumerator /* final */ {
  enum option_t /* : unsigned char */ {
    DIGITS,  // ->> Renumeration index digits;                                           Defaults to `"0123456789"`
    HELP,    // ->> Display help text;                                                   Defaults to `false`
    HIDDEN,  // ->> Accept hidden files in list of indexed files;                        Defaults to `false`
    INDEX,   // ->> Starting renumeration index;                                         Defaults to `"1"`
    INPUT,   // ->> `std::printf(…)`-style formatted filter for indexed files to accept; Defaults to `NULL`
    MEMORY,  // ->> Maximum memory allocation;                                           Defaults to `SIZE_MAX`
    OUTPUT,  // ->> `std::printf(…)`-style formatted filter for renamed file names;      Defaults to `NULL`
    PADDING, // ->> Formats renumeration index with leading digits (zeroes);             Defaults to `"0"`
    SILENT,  // ->> Silences log displays;                                               Defaults to `false`
    SYSTEM   // ->> Accept system files in list of indexed files;                        Defaults to `false`
  };

  struct switch_option_t /* final */ {
    char const                *longNames;
    char const                *shortNames;
    enum renumerator::option_t value;
  };

  struct switch_t /* final */ {
    struct renumerator::switch_option_t option;
    union /* --> alignas(std::max_align_t) unsigned char[…] */ {
      enum /* : std::size_t */ { length = /* --> max(…) */ sizeof(long double) | sizeof(std::size_t) | sizeof(void*) | sizeof(void (*)(...)) | sizeof(void (switch_t::*)(...)) };
      typedef unsigned char type[length];

      type _;
      union {
        long double            alignmentA;
        std::size_t            alignmentB;
        void                  *alignmentC;
        void                 (*alignmentD)(...);
        void      (switch_t:: *alignmentE)(...);
        #if defined _MSVC_LANG /* --> 201402L+ */ or __cplusplus >= 201103L
          std::max_align_t alignmentF;
        #endif
      };

      /* constexpr */ inline operator type&() /* noexcept */ { return this -> _; }
    } buffer;
    void *value;
    bool  active;
  };
...
int main(int count, char* arguments[]) /* noexcept */ {
  struct renumerator::switch_option_t const commandLineOptions[] = {
    {"digits"              "\0", "d" "\0", renumerator::DIGITS},
    {"help"                "\0", "h" "\0", renumerator::HELP},
    {"hidden"              "\0", NULL,     renumerator::HIDDEN},
    {"index"               "\0", "i" "\0", renumerator::INDEX},
    {"filter" "\0" "input" "\0", "f" "\0", renumerator::INPUT},
    {"memory"              "\0", "m" "\0", renumerator::MEMORY},
    {"output"              "\0", "o" "\0", renumerator::OUTPUT},
    {"padding"             "\0", "p" "\0", renumerator::PADDING},
    {"silent"              "\0", "s" "\0", renumerator::SILENT},
    {"system"              "\0", NULL,     renumerator::SYSTEM}
  };

  struct renumerator::switch_t commandLineSwitches[sizeof commandLineOptions / sizeof(struct renumerator::switch_option_t)] = {};
  char const            *const programName                                                                                  = "renum";
  char const                  *renumerationDirectory                                                                        = NULL;

  /* ... */
  for (std::size_t index = sizeof commandLineSwitches / sizeof(struct renumerator::switch_t); index--; ) {
    commandLineSwitches[index].active = false;
    commandLineSwitches[index].option = commandLineOptions[index];
    commandLineSwitches[index].value  = NULL;

    for (unsigned char *iterator = commandLineSwitches[index].buffer + commandLineSwitches[index].buffer.length; iterator != commandLineSwitches[index].buffer; )
    *--iterator = 0x00u;
  }

  commandLineSwitches[renumerator::DIGITS] .value = ::new (commandLineSwitches[renumerator::DIGITS] .buffer) char const*("0123456789");
  commandLineSwitches[renumerator::INDEX]  .value = ::new (commandLineSwitches[renumerator::INDEX]  .buffer) char const*("1");
  commandLineSwitches[renumerator::INPUT]  .value = ::new (commandLineSwitches[renumerator::INPUT]  .buffer) char const*(NULL);
  commandLineSwitches[renumerator::MEMORY] .value = ::new (commandLineSwitches[renumerator::MEMORY] .buffer) std::size_t(-1); // --> SIZE_MAX
  commandLineSwitches[renumerator::OUTPUT] .value = ::new (commandLineSwitches[renumerator::OUTPUT] .buffer) char const*(NULL);
  commandLineSwitches[renumerator::PADDING].value = ::new (commandLineSwitches[renumerator::PADDING].buffer) char const*("0");
  ...
  for (unsigned index = 0u; NULL != *arguments and index != static_cast<unsigned>(count > 0 ? count : 0); ++arguments) /* ->> US-ASCII encoding */ {
    ...
    if (not commandLineError and **arguments == '-') {
      ...
      char const* renumerator::switch_option_t::     *const commandLineOptionNames = LONG_OPTION == commandLineOptionType ? &renumerator::switch_option_t::longNames : &renumerator::switch_option_t::shortNames;

      // ...
      commandLineError = "Unrecognized command-line option";

      for (struct renumerator::switch_t *iterator = commandLineSwitches; iterator != commandLineSwitches + (sizeof commandLineSwitches / sizeof(struct renumerator::switch_t)); ++iterator)
        ...
        if ('\0' == *names) { // ->> Delimited `name` prefixes `argument`
          ...
          if (*argument == (NULL != iterator -> value ? '=' : '\0')) {
            if (NULL != iterator -> value) {
              ...
              // ...
              if (iterator == commandLineSwitches + renumerator::MEMORY) {
                switch (*argument) // [0-9]+
